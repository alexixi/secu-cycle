...
\noindent L’objectif de ce projet est de proposer des itinéraires cyclables sécurisés sur la zone géographique du sud bordelais, entre la place de la Victoire et le Campus universitaire de Talence/Pessac (Figure~\ref{fig:cartepb}). 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{carte_problematique.pdf}
    \caption{Carte représentant la zone étudiée.}
    \label{fig:cartepb}
\end{figure}

\noindent Cet itinéraire est emprunté chaque jour par de nombreux étudiants, principalement en tramway via la ligne B.
...
Pendant l’année 2024, la pratique du vélo a augmenté de 6\% par rapport à 2023, en suivant la tendance des dernières années~\cite{ObservatoireVelo}.
...
Le cycliste est un usager fragile et les accidents peuvent arriver.
Il est donc important de séparer les usagers le plus possible pour éviter les sources d’incidents.\\

\subsection{Objectifs}
\label{sec:objectifs}
\noindent L'objectif du projet est de développer un site web et une application, faciles d'utilisations et ayant une interface intuitive, en utilisant une méthodologie de Génie Logiciel.\\

\noindent L’objectif du site et de l’application est de proposer l’itinéraire qui convient le mieux en terme de compromis entre la sécurité et le temps de trajet, en s’adaptant au profil de l’utilisateur.\\
...
Ainsi, le site et l'application proposent des trajets qui peuvent convenir à chacun, pour permettre à tous une pratique sécurisée du vélo.\\

\noindent Pour toucher un maximum d'utilisateurs, le projet consiste au développement d'un site web mais aussi d'une application mobile.
Via ces deux interfaces, l'utilisateur peut créer son compte, afin de stocker ses informations.
...
Celui-ci décrit les différents objectifs fixés au début du projet, et contient un diagramme de Gant (Figure~\ref{fig:gant}) pour organiser les différentes tâches à effectuer.
...
\noindent Une première équipe, en jaune sur le diagramme de la Figure~\ref{fig:gant}, composée de Joan Dumarchat et Léia Daragnès, a pris en charge le calcul des itinéraires sécurisés.
...
\noindent Une seconde équipe, représentée en violet, composée de Khaoula Najmeddine et Angelo Tunney, s'est concentrée sur l'API.
...
\noindent Une troisième et dernière équipe, en bleu sur le diagramme, composée de Alexis Gaudray Bouju et Matheline Chevalier, s'est occupée du développement Frontend.
...
\noindent De plus, pour pouvoir suivre l'avancement de chaque équipe, nous avons créé une fiche de suivi du projet~\cite{ficheSuivi}.
Sur celle-ci se trouve le détail de chaque tâche à effectuer, ainsi que l'avancement de chaque équipe.
Comme on peut le voir sur cette fiche, par manque de temps, toutes les pistes évoquées lors de l'élaboration du cahier des charges n'ont pas pu être abouties.
Il existe donc de nombreuses pistes pour l'amélioration future du projet, comme l'intégration de la météo, des données de pollution de l'air ou encore la fidélisation des utilisateurs.\\
