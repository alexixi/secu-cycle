// Contenu éditorial français des cartes thématiques.
//
// Séparé du registre langue-neutre (thematicMapsCore.js) pour deux raisons :
//
//   - le poids. PAGE_CONTENT pèse l'essentiel du chunk des pages carte. Mêler les
//     langues dans un même objet le ferait télécharger deux fois par tout le monde ;
//     et comme il est indexé dynamiquement, aucun tree-shaking ne peut l'éviter. Un
//     fichier par langue, atteint par un import() distinct, donne un chunk par langue.
//
//   - la rédaction. Le contenu est écrit à la main pour chaque couple ville × thème.
//     Aucun texte généré par gabarit : une page qui n'a rien d'unique à dire ne doit
//     pas être publiée. Cette règle vaut pour toute langue ajoutée ici.
//
// `prep` et `de` portent la grammaire française (« à Bordeaux », « de Bordeaux ») et
// n'ont pas d'équivalent dans les autres langues, qui composent leurs gabarits
// autrement. Le contrat entre langues porte sur les CLÉS de PAGE_CONTENT, pas sur la
// liste des champs.
//
// Contrainte identique au reste du registre : JavaScript pur, importable par Node.

// Sources redéfinies par certaines pages : elles sont langue-neutre (noms propres,
// licences, URL) et vivent donc dans le socle.
import {
    SOURCE_BAAC,
...
// Énumération « A, B et C ». Intl.ListFormat porte la règle de chaque langue, y compris
// la place et la forme de la conjonction.
export const listFormat = (noms) =>
    new Intl.ListFormat('fr', { style: 'long', type: 'conjunction' }).format(noms);

// Gabarits de titre dérivés d'une ville. Ils vivent ici parce qu'ils dépendent de la
// grammaire : le français colle une préposition fléchie, l'anglais non.
export const cityHubTitle = (city) => `Cartes cyclables ${city.prep}`;
